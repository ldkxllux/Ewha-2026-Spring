...
| [18](#team-18) | 디바트(deep-art) | 연구 | 철새 군집 이동을 활용한 XAI 시각화 미디어아트 |
...
| 항목 | 내용 |
| --- | --- |
| 프로젝트명 | 철새 군집 이동을 활용한 XAI 시각화 미디어아트 |
| 트랙 | 연구 |
| 팀명 | 디바트(deep-art) |
| 팀구성 | 최현서, 이나겸, 김나경 |
| 팀지도교수 | 박현석 |
| 무엇을 만들고자 하는가 | 영상예술학과 대학원생들과 협업하여 제작하는 인터랙티브 미디어아트 설치 작품. 실제 철새의 GPS 데이터를 학습시켜 이동 경로를 예측하고 이를 실시간으로 렌더링한다.<br><br>**[시각화 연출]**<br>화면 배경에는 AI가 연산한 복수의 후보 경로들이 옅은 빛의 선으로 펼쳐지며, 가장 확률이 높은 최적 경로가 굵은 선으로 강조된다. 이 굵은 선을 따라 수천 마리의 철새 군집이 비행한다.<br><br>**[인터랙티브(관객 참여 요소)]**<br>관객이 '풍속'과 같은 환경 변수를 실시간으로 조작하면 XAI 기법을 활용하여 이 변수가 확률 변동에 미친 기여도를 수치화하고, 이를 경로 선택에 즉시 반영한다. |
| 고객 (누구를 위해) | AI의 작동 방식에 대해 이해하고자 하는 대중 및 전시 관람객 |
| Pain Point (해결할 문제) | AI는 이미 일상 곳곳에 쓰이고 있지만, 그 판단 과정이 숨겨져 있어(블랙박스) 사람들은 AI의 결과를 무조건적인 '정답'으로 받아들이기 쉽다. 하지만 AI는 세계를 완벽히 설명하는 것이 아니라, 수많은 가능성 중에서 확률이 가장 높은 선택지를 고를 뿐이다.<br><br>기존의 XAI(설명 가능한 AI)는 이 과정을 설명해 주지만, 주로 전공자를 위한 복잡한 수치나 그래프로 되어 있어 일반 대중이 이해하기는 어렵다. 이 작품은 미디어아트를 활용해 이 문제를 해결하고자 한다. 복잡한 수치 대신 직관적이고 시각적으로 아름다운 결과물을 제시하여, 대중이 심리적인 거부감 없이 AI의 원리를 쉽게 받아들일 수 있도록 진입 장벽을 낮춘다. |
| 사용 기술 | LSTM(시계열 경로 확률 예측), SHAP(환경 변수 기여도 분석), Boids 알고리즘(군집 시뮬레이션), FastAPI(데이터 통신), Unity VFX Graph(실시간 렌더링) |
| 기대 효과 | **[연구적 측면]**<br>예술적 시각화 도구가 일반 대중의 AI 블랙박스 이해도와 리터러시 향상에 미치는 영향을 탐구한다.<br><br>**[예술적 측면]**<br>관객에게 데이터가 예술로 승화되는 심미적 경험과 기술-자연의 관계성에 대한 철학적 사유를 제공한다. |
